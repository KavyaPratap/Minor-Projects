#removed treeview---------successfully!!.....will added in future
#refined save and load layout, now we also have a png file to see what file contains.
#
...
class TkinterAppDesigner:
    ...
    def save_layout(self, event=None):
        ...
        # Take a screenshot of the layout
        canvas_bbox = self.canvas.bbox("all")
        x1, y1, x2, y2 = self.canvas.winfo_rootx(), self.canvas.winfo_rooty(), self.canvas.winfo_rootx() + canvas_bbox[2], self.canvas.winfo_rooty() + canvas_bbox[3]
        screenshot = ImageGrab.grab(bbox=(x1, y1, x2, y2))

        # Convert screenshot to base64 and include in layout data
        screenshot_buffer = io.BytesIO()
        screenshot.save(screenshot_buffer, format="PNG")
        layout_data["screenshot"] = base64.b64encode(screenshot_buffer.getvalue()).decode("utf-8")
        ...
        if file_path:
            ...
            # Optionally, save a separate PNG file for external viewing
            screenshot.save(file_path.replace(".kpfile", ".png"))
    ...
    def load_layout(self, file_path=None):
        ...
        if file_path:
            try:
                ...
                # Ignore the screenshot when loading layout
                if "screenshot" in layout_data:
                    del layout_data["screenshot"]
    ...
    def generate_code(self, event=None):
        ...
        for widget in self.widgets:
            ...
            if isinstance(widget, tk.Label) and hasattr(widget, "image"):
                ...
                # Start widget declaration
                widget_code = f"{widget_name} = tk.{widget_type}(root"
                properties = []

                # Add relevant widget properties
                if widget_type in ["Button", "Label"]:
                    properties.append(f"text='{widget.cget('text')}'")
                    properties.append(f"bg='{widget.cget('bg')}'")
                    properties.append(f"fg='{widget.cget('fg')}'")
                elif widget_type == "Entry":
                    properties.append(f"bg='{widget.cget('bg')}'")
                    properties.append(f"width={widget.cget('width')}")
                elif widget_type == "Frame":
                    properties.append(f"bg='{widget.cget('bg')}'")

                # Add command property if it exists
                if widget in self.widget_functions and isinstance(self.widget_functions[widget], dict):
                    for function_name, function_code in self.widget_functions[widget].items():
                        ...
                        properties.append(f"command={function_name}")

                # Complete the widget declaration with properties
                widget_code += ", " + ", ".join(properties) + ")\n" if properties else ")\n"
                widget_declarations += widget_code

            # Get widget coordinates and add placement
            coords = self.canvas.coords(self.widget_ids[widget]) if widget in self.widget_ids else (0, 0)
            x, y = int(coords[0]), int(coords[1])
            widget_placements += f"{widget_name}.place(x={x}, y={y}, width={widget.winfo_width()}, height={widget.winfo_height()})\n"
...
# will def clipboard_clear() and clipboard_append() in next version........
    ...
    def save_generated_code(self):
        # Generate the code using the existing logic
